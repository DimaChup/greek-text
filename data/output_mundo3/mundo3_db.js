// Generated word frequency database with word numbers for mundo3
const mundo3Database = {
    'de': {
      wordNumber: 1,
      frequency: 29,
      partOfSpeech: 'Preposition',
      morphology: '',
      meanings: ['of', 'from', 'about'],
      bestTranslation: 'of',
      lemma: 'de',
      LemmaMeanings: ['of', 'from', 'about', 'concerning']
    },
    'en': {
      wordNumber: 2,
      frequency: 14,
      partOfSpeech: 'Preposition',
      morphology: '',
      meanings: ['in', 'on', 'at'],
      bestTranslation: 'in',
      lemma: 'en',
      LemmaMeanings: ['in', 'on', 'at', 'within', 'during']
    },
    'y': {
      wordNumber: 3,
      frequency: 12,
      partOfSpeech: 'Conjunction',
      morphology: '',
      meanings: ['and'],
      bestTranslation: 'and',
      lemma: 'y',
      LemmaMeanings: ['and', 'plus', 'as well as', 'also']
    },
    'el': {
      wordNumber: 4,
      frequency: 11,
      partOfSpeech: 'Article',
      morphology: 'Masculine Singular',
      meanings: ['the'],
      bestTranslation: 'the',
      lemma: 'el',
      LemmaMeanings: ['the', 'him', 'it', 'that one']
    },
    'la': {
      wordNumber: 5,
      frequency: 11,
      partOfSpeech: 'Article',
      morphology: 'Feminine Singular',
      meanings: ['the'],
      bestTranslation: 'the',
      lemma: 'la',
      LemmaMeanings: ['the', 'her', 'it', 'that one']
    },
    'que': {
      wordNumber: 6,
      frequency: 10,
      partOfSpeech: 'Conjunction/Pronoun',
      morphology: '',
      meanings: ['that', 'which', 'who'],
      bestTranslation: 'that',
      lemma: 'que',
      LemmaMeanings: ['that', 'which', 'who', 'what', 'than']
    },
    'los': {
      wordNumber: 7,
      frequency: 8,
      partOfSpeech: 'Article',
      morphology: 'Masculine Plural',
      meanings: ['the'],
      bestTranslation: 'the',
      lemma: 'el',
      LemmaMeanings: ['the', 'them', 'those', 'ones']
    },
    'del': {
      wordNumber: 8,
      frequency: 6,
      partOfSpeech: 'Preposition + Article',
      morphology: 'Contraction of de + el',
      meanings: ['of the', 'from the'],
      bestTranslation: 'of the',
      lemma: 'del',
      LemmaMeanings: ['of the', 'from the', 'about the', 'belonging to the']
    },
    'una': {
      wordNumber: 9,
      frequency: 5,
      partOfSpeech: 'Article',
      morphology: 'Feminine Singular',
      meanings: ['a', 'one'],
      bestTranslation: 'a',
      lemma: 'un',
      LemmaMeanings: ['a', 'one', 'any', 'approximately']
    },
    'se': {
      wordNumber: 10,
      frequency: 5,
      partOfSpeech: 'Pronoun',
      morphology: 'Reflexive',
      meanings: ['oneself', 'himself', 'herself', 'itself', 'themselves'],
      bestTranslation: 'oneself',
      lemma: 'se',
      LemmaMeanings: ['oneself', 'himself', 'herself', 'themselves', 'each other']
    },
    'a': {
      wordNumber: 11,
      frequency: 5,
      partOfSpeech: 'Preposition',
      morphology: '',
      meanings: ['to', 'at'],
      bestTranslation: 'to',
      lemma: 'a',
      LemmaMeanings: ['to', 'at', 'by', 'for', 'towards']
    },
    'mundo': {
      wordNumber: 12,
      frequency: 5,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['world', 'earth', 'globe'],
      bestTranslation: 'world',
      lemma: 'mundo',
      LemmaMeanings: ['world', 'earth', 'globe', 'universe', 'mankind']
    },
    'vez': {
      wordNumber: 13,
      frequency: 4,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['time', 'occasion', 'instance'],
      bestTranslation: 'time',
      lemma: 'vez',
      LemmaMeanings: ['time', 'occasion', 'instance', 'turn', 'opportunity']
    },
    'me': {
      wordNumber: 14,
      frequency: 4,
      partOfSpeech: 'Pronoun',
      morphology: 'Personal Object',
      meanings: ['me', 'to me', 'myself'],
      bestTranslation: 'me',
      lemma: 'me',
      LemmaMeanings: ['me', 'to me', 'myself', 'for me', 'with me']
    },
    'por': {
      wordNumber: 15,
      frequency: 4,
      partOfSpeech: 'Preposition',
      morphology: '',
      meanings: ['for', 'by', 'through'],
      bestTranslation: 'for',
      lemma: 'por',
      LemmaMeanings: ['for', 'by', 'through', 'because of', 'along']
    },
    'un': {
      wordNumber: 16,
      frequency: 4,
      partOfSpeech: 'Article',
      morphology: 'Masculine Singular',
      meanings: ['a', 'one'],
      bestTranslation: 'a',
      lemma: 'un',
      LemmaMeanings: ['a', 'one', 'any', 'some', 'approximately']
    },
    'pero': {
      wordNumber: 17,
      frequency: 4,
      partOfSpeech: 'Conjunction',
      morphology: '',
      meanings: ['but', 'yet', 'however'],
      bestTranslation: 'but',
      lemma: 'pero',
      LemmaMeanings: ['but', 'yet', 'however', 'nevertheless', 'though']
    },
    'al': {
      wordNumber: 18,
      frequency: 3,
      partOfSpeech: 'Preposition + Article',
      morphology: 'Contraction of a + el',
      meanings: ['to the', 'at the'],
      bestTranslation: 'to the',
      lemma: 'al',
      LemmaMeanings: ['to the', 'at the', 'upon the', 'by the', 'when']
    },
    'libro': {
      wordNumber: 19,
      frequency: 3,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['book', 'volume'],
      bestTranslation: 'book',
      lemma: 'libro',
      LemmaMeanings: ['book', 'volume', 'text', 'work', 'publication']
    },
    'chatwin': {
      wordNumber: 20,
      frequency: 3,
      partOfSpeech: 'Noun',
      morphology: 'Proper Noun',
      meanings: ['Chatwin (author surname)'],
      bestTranslation: 'Chatwin',
      lemma: 'Chatwin',
      LemmaMeanings: ['Chatwin', 'Bruce Chatwin', 'the author Chatwin', 'the writer Chatwin']
    },
    'luego': {
      wordNumber: 21,
      frequency: 3,
      partOfSpeech: 'Adverb',
      morphology: '',
      meanings: ['then', 'later', 'afterwards'],
      bestTranslation: 'then',
      lemma: 'luego',
      LemmaMeanings: ['then', 'later', 'afterwards', 'subsequently', 'next']
    },
    'más': {
      wordNumber: 22,
      frequency: 3,
      partOfSpeech: 'Adverb',
      morphology: '',
      meanings: ['more', 'most', 'plus'],
      bestTranslation: 'more',
      lemma: 'más',
      LemmaMeanings: ['more', 'most', 'plus', 'further', 'additionally']
    },
    'años': {
      wordNumber: 23,
      frequency: 3,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Plural',
      meanings: ['years'],
      bestTranslation: 'years',
      lemma: 'año',
      LemmaMeanings: ['year', 'annual period', 'twelve months', 'calendar year']
    },
    'las': {
      wordNumber: 24,
      frequency: 3,
      partOfSpeech: 'Article',
      morphology: 'Feminine Plural',
      meanings: ['the'],
      bestTranslation: 'the',
      lemma: 'la',
      LemmaMeanings: ['the', 'them', 'those', 'ones']
    },
    'llamadme': {
      wordNumber: 25,
      frequency: 2,
      partOfSpeech: 'Verb',
      morphology: 'Imperative Second Person Plural with direct object pronoun me',
      meanings: ['call me'],
      bestTranslation: 'call me',
      lemma: 'llamar',
      LemmaMeanings: ['to call', 'to name', 'to telephone', 'to beckon', 'to summon']
    },
    'ismael': {
      wordNumber: 26,
      frequency: 2,
      partOfSpeech: 'Noun',
      morphology: 'Proper Noun',
      meanings: ['Ishmael', 'Ismael (name)'],
      bestTranslation: 'Ishmael',
      lemma: 'Ismael',
      LemmaMeanings: ['Ishmael', 'Ismael', 'biblical figure Ishmael', 'male name']
    },
    'veces': {
      wordNumber: 27,
      frequency: 2,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Plural',
      meanings: ['times', 'occasions'],
      bestTranslation: 'times',
      lemma: 'vez',
      LemmaMeanings: ['time', 'occasion', 'instance', 'turn', 'opportunity']
    },
    'aeropuerto': {
      wordNumber: 28,
      frequency: 2,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['airport'],
      bestTranslation: 'airport',
      lemma: 'aeropuerto',
      LemmaMeanings: ['airport', 'air terminal', 'airfield', 'aviation hub']
    },
    'hamburgo': {
      wordNumber: 29,
      frequency: 2,
      partOfSpeech: 'Noun',
      morphology: 'Proper Noun',
      meanings: ['Hamburg'],
      bestTranslation: 'Hamburg',
      lemma: 'Hamburgo',
      LemmaMeanings: ['Hamburg', 'city in Germany', 'German port city', 'Hanseatic city']
    },
    'cada': {
      wordNumber: 30,
      frequency: 2,
      partOfSpeech: 'Adjective/Determiner',
      morphology: '',
      meanings: ['each', 'every'],
      bestTranslation: 'each',
      lemma: 'cada',
      LemmaMeanings: ['each', 'every', 'apiece', 'per', 'individual']
    },
    'peso': {
      wordNumber: 31,
      frequency: 2,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['weight', 'burden', 'currency'],
      bestTranslation: 'weight',
      lemma: 'peso',
      LemmaMeanings: ['weight', 'burden', 'heaviness', 'gravity', 'importance']
    },
    'acercaba': {
      wordNumber: 32,
      frequency: 2,
      partOfSpeech: 'Verb',
      morphology: 'Imperfect Indicative Third Person Singular',
      meanings: ['was approaching', 'was coming near'],
      bestTranslation: 'was approaching',
      lemma: 'acercar',
      LemmaMeanings: ['to approach', 'to bring near', 'to draw closer', 'to move towards', 'to get closer']
    },
    'hora': {
      wordNumber: 33,
      frequency: 2,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['hour', 'time', 'o\'clock'],
      bestTranslation: 'hour',
      lemma: 'hora',
      LemmaMeanings: ['hour', 'time', 'o\'clock', 'moment', 'period']
    },
    'salida': {
      wordNumber: 34,
      frequency: 2,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['exit', 'departure', 'way out'],
      bestTranslation: 'departure',
      lemma: 'salida',
      LemmaMeanings: ['exit', 'departure', 'way out', 'outlet', 'solution']
    },
    'embarque': {
      wordNumber: 35,
      frequency: 2,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['boarding', 'embarkation'],
      bestTranslation: 'boarding',
      lemma: 'embarque',
      LemmaMeanings: ['boarding', 'embarkation', 'shipment', 'loading', 'departure']
    },
    'llevaba': {
      wordNumber: 36,
      frequency: 2,
      partOfSpeech: 'Verb',
      morphology: 'Imperfect Indicative First/Third Person Singular',
      meanings: ['was carrying', 'was wearing', 'had been'],
      bestTranslation: 'was carrying',
      lemma: 'llevar',
      LemmaMeanings: ['to carry', 'to take', 'to wear', 'to bear', 'to lead']
    },
    'patagonia': {
      wordNumber: 37,
      frequency: 2,
      partOfSpeech: 'Noun',
      morphology: 'Proper Noun',
      meanings: ['Patagonia'],
      bestTranslation: 'Patagonia',
      lemma: 'Patagonia',
      LemmaMeanings: ['Patagonia', 'Patagonian region', 'southern South America', 'Argentina/Chile region']
    },
    'siempre': {
      wordNumber: 38,
      frequency: 2,
      partOfSpeech: 'Adverb',
      morphology: '',
      meanings: ['always', 'forever', 'ever'],
      bestTranslation: 'always',
      lemma: 'siempre',
      LemmaMeanings: ['always', 'forever', 'ever', 'constantly', 'perpetually']
    },
    'o': {
      wordNumber: 39,
      frequency: 2,
      partOfSpeech: 'Conjunction',
      morphology: '',
      meanings: ['or', 'either'],
      bestTranslation: 'or',
      lemma: 'o',
      LemmaMeanings: ['or', 'either', 'otherwise', 'alternatively', 'else']
    },
    'estaba': {
      wordNumber: 40,
      frequency: 2,
      partOfSpeech: 'Verb',
      morphology: 'Imperfect Indicative First/Third Person Singular',
      meanings: ['was', 'was being', 'was located'],
      bestTranslation: 'was',
      lemma: 'estar',
      LemmaMeanings: ['to be', 'to stay', 'to remain', 'to exist', 'to be located']
    },
    'pensaba': {
      wordNumber: 41,
      frequency: 2,
      partOfSpeech: 'Verb',
      morphology: 'Imperfect Indicative First/Third Person Singular',
      meanings: ['was thinking', 'was considering'],
      bestTranslation: 'was thinking',
      lemma: 'pensar',
      LemmaMeanings: ['to think', 'to believe', 'to consider', 'to plan', 'to reflect']
    },
    'hasta': {
      wordNumber: 42,
      frequency: 2,
      partOfSpeech: 'Preposition',
      morphology: '',
      meanings: ['until', 'up to', 'even'],
      bestTranslation: 'until',
      lemma: 'hasta',
      LemmaMeanings: ['until', 'up to', 'even', 'as far as', 'to the point of']
    },
    'chile': {
      wordNumber: 43,
      frequency: 2,
      partOfSpeech: 'Noun',
      morphology: 'Proper Noun',
      meanings: ['Chile'],
      bestTranslation: 'Chile',
      lemma: 'Chile',
      LemmaMeanings: ['Chile', 'Republic of Chile', 'South American country', 'Chilean nation']
    },
    'deseos': {
      wordNumber: 44,
      frequency: 2,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Plural',
      meanings: ['desires', 'wishes', 'longings'],
      bestTranslation: 'desires',
      lemma: 'deseo',
      LemmaMeanings: ['desire', 'wish', 'longing', 'craving', 'yearning']
    },
    'allá': {
      wordNumber: 45,
      frequency: 2,
      partOfSpeech: 'Adverb',
      morphology: '',
      meanings: ['there', 'over there', 'yonder'],
      bestTranslation: 'there',
      lemma: 'allá',
      LemmaMeanings: ['there', 'over there', 'yonder', 'in that place', 'far away']
    },
    'repetida': {
      wordNumber: 46,
      frequency: 2,
      partOfSpeech: 'Adjective',
      morphology: 'Feminine Singular',
      meanings: ['repeated', 'reiterated'],
      bestTranslation: 'repeated',
      lemma: 'repetido',
      LemmaMeanings: ['repeated', 'reiterated', 'recurrent', 'frequent', 'habitual']
    },
    'creí': {
      wordNumber: 47,
      frequency: 2,
      partOfSpeech: 'Verb',
      morphology: 'Preterite Indicative First Person Singular',
      meanings: ['I believed', 'I thought'],
      bestTranslation: 'I believed',
      lemma: 'creer',
      LemmaMeanings: ['to believe', 'to think', 'to trust', 'to have faith', 'to suppose']
    },
    'fin': {
      wordNumber: 48,
      frequency: 2,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['end', 'conclusion', 'purpose'],
      bestTranslation: 'end',
      lemma: 'fin',
      LemmaMeanings: ['end', 'conclusion', 'purpose', 'goal', 'finish']
    },
    'como': {
      wordNumber: 49,
      frequency: 2,
      partOfSpeech: 'Conjunction/Adverb',
      morphology: '',
      meanings: ['as', 'like', 'how'],
      bestTranslation: 'as',
      lemma: 'como',
      LemmaMeanings: ['as', 'like', 'how', 'such as', 'in the way that']
    },
    '1': {
      wordNumber: 50,
      frequency: 1,
      partOfSpeech: 'Numeral',
      morphology: 'Cardinal',
      meanings: ['one', 'first'],
      bestTranslation: 'one',
      lemma: '1',
      LemmaMeanings: ['one', 'first', 'single', 'primary', 'initial']
    },
    'repetí': {
      wordNumber: 51,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Preterite Indicative First Person Singular',
      meanings: ['I repeated', 'I reiterated'],
      bestTranslation: 'I repeated',
      lemma: 'repetir',
      LemmaMeanings: ['to repeat', 'to reiterate', 'to recite', 'to reproduce', 'to do again']
    },
    'varias': {
      wordNumber: 52,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Feminine Plural',
      meanings: ['several', 'various', 'different'],
      bestTranslation: 'several',
      lemma: 'vario',
      LemmaMeanings: ['various', 'several', 'different', 'diverse', 'changeable']
    },
    'mientras': {
      wordNumber: 53,
      frequency: 1,
      partOfSpeech: 'Conjunction',
      morphology: '',
      meanings: ['while', 'whereas', 'as long as'],
      bestTranslation: 'while',
      lemma: 'mientras',
      LemmaMeanings: ['while', 'whereas', 'as long as', 'during the time that', 'meanwhile']
    },
    'esperaba': {
      wordNumber: 54,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Imperfect Indicative First/Third Person Singular',
      meanings: ['was waiting', 'was hoping', 'was expecting'],
      bestTranslation: 'was waiting',
      lemma: 'esperar',
      LemmaMeanings: ['to wait', 'to hope', 'to expect', 'to anticipate', 'to look forward to']
    },
    'sentía': {
      wordNumber: 55,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Imperfect Indicative First/Third Person Singular',
      meanings: ['was feeling', 'was sensing'],
      bestTranslation: 'was feeling',
      lemma: 'sentir',
      LemmaMeanings: ['to feel', 'to sense', 'to regret', 'to perceive', 'to experience']
    },
    'fuerza': {
      wordNumber: 56,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['force', 'strength', 'power'],
      bestTranslation: 'force',
      lemma: 'fuerza',
      LemmaMeanings: ['force', 'strength', 'power', 'might', 'vigor']
    },
    'extraña': {
      wordNumber: 57,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Feminine Singular',
      meanings: ['strange', 'odd', 'weird'],
      bestTranslation: 'strange',
      lemma: 'extraño',
      LemmaMeanings: ['strange', 'odd', 'weird', 'unusual', 'foreign']
    },
    'otorgaba': {
      wordNumber: 58,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Imperfect Indicative Third Person Singular',
      meanings: ['was granting', 'was giving', 'was awarding'],
      bestTranslation: 'was granting',
      lemma: 'otorgar',
      LemmaMeanings: ['to grant', 'to give', 'to award', 'to bestow', 'to confer']
    },
    'mayor': {
      wordNumber: 59,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Comparative',
      meanings: ['greater', 'larger', 'older'],
      bestTranslation: 'greater',
      lemma: 'grande',
      LemmaMeanings: ['big', 'large', 'great', 'grand', 'important']
    },
    'delgado': {
      wordNumber: 60,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Masculine Singular',
      meanings: ['thin', 'slim', 'slender'],
      bestTranslation: 'thin',
      lemma: 'delgado',
      LemmaMeanings: ['thin', 'slim', 'slender', 'lean', 'skinny']
    },
    'cuadernillo': {
      wordNumber: 61,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['booklet', 'notebook', 'pamphlet'],
      bestTranslation: 'booklet',
      lemma: 'cuadernillo',
      LemmaMeanings: ['booklet', 'notebook', 'pamphlet', 'brochure', 'small copybook']
    },
    'pasaje': {
      wordNumber: 62,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['passage', 'ticket', 'fare'],
      bestTranslation: 'ticket',
      lemma: 'pasaje',
      LemmaMeanings: ['passage', 'ticket', 'fare', 'excerpt', 'corridor']
    },
    'aumentaba': {
      wordNumber: 63,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Imperfect Indicative Third Person Singular',
      meanings: ['was increasing', 'was growing', 'was augmenting'],
      bestTranslation: 'was increasing',
      lemma: 'aumentar',
      LemmaMeanings: ['to increase', 'to augment', 'to grow', 'to raise', 'to enlarge']
    },
    'conforme': {
      wordNumber: 64,
      frequency: 1,
      partOfSpeech: 'Adjective/Adverb/Conjunction',
      morphology: '',
      meanings: ['according to', 'as', 'satisfied'],
      bestTranslation: 'as',
      lemma: 'conforme',
      LemmaMeanings: ['according to', 'as', 'satisfied', 'in agreement', 'conforming']
    },
    'había': {
      wordNumber: 65,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Imperfect Indicative Third Person Singular',
      meanings: ['there was', 'had'],
      bestTranslation: 'there was',
      lemma: 'haber',
      LemmaMeanings: ['to have', 'to exist', 'to be', 'to occur', 'auxiliary verb']
    },
    'atravesado': {
      wordNumber: 66,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Past Participle',
      meanings: ['crossed', 'traversed', 'passed through'],
      bestTranslation: 'crossed',
      lemma: 'atravesar',
      LemmaMeanings: ['to cross', 'to traverse', 'to pass through', 'to pierce', 'to go through']
    },
    'primer': {
      wordNumber: 67,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Masculine Singular Ordinal',
      meanings: ['first', 'primary', 'initial'],
      bestTranslation: 'first',
      lemma: 'primero',
      LemmaMeanings: ['first', 'primary', 'initial', 'earliest', 'foremost']
    },
    'control': {
      wordNumber: 68,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['control', 'check', 'checkpoint'],
      bestTranslation: 'control',
      lemma: 'control',
      LemmaMeanings: ['control', 'check', 'checkpoint', 'supervision', 'command']
    },
    'paseaba': {
      wordNumber: 69,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Imperfect Indicative First/Third Person Singular',
      meanings: ['was walking', 'was strolling'],
      bestTranslation: 'was walking',
      lemma: 'pasear',
      LemmaMeanings: ['to walk', 'to stroll', 'to promenade', 'to wander', 'to take a walk']
    },
    'sala': {
      wordNumber: 70,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['room', 'hall', 'lounge'],
      bestTranslation: 'room',
      lemma: 'sala',
      LemmaMeanings: ['room', 'hall', 'lounge', 'living room', 'chamber']
    },
    'aferrado': {
      wordNumber: 71,
      frequency: 1,
      partOfSpeech: 'Adjective/Verb',
      morphology: 'Past Participle',
      meanings: ['clutched', 'clinging to', 'holding on'],
      bestTranslation: 'clutching',
      lemma: 'aferrar',
      LemmaMeanings: ['to clutch', 'to cling to', 'to grasp', 'to hold on', 'to grip']
    },
    'bolso': {
      wordNumber: 72,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['bag', 'handbag', 'purse'],
      bestTranslation: 'bag',
      lemma: 'bolso',
      LemmaMeanings: ['bag', 'handbag', 'purse', 'tote', 'satchel']
    },
    'mano': {
      wordNumber: 73,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['hand'],
      bestTranslation: 'hand',
      lemma: 'mano',
      LemmaMeanings: ['hand', 'handwriting', 'side', 'worker', 'help']
    },
    'no': {
      wordNumber: 74,
      frequency: 1,
      partOfSpeech: 'Adverb',
      morphology: '',
      meanings: ['no', 'not'],
      bestTranslation: 'no',
      lemma: 'no',
      LemmaMeanings: ['no', 'not', 'non-', 'negative', 'refusal']
    },
    'demasiadas': {
      wordNumber: 75,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Feminine Plural',
      meanings: ['too many', 'excessive'],
      bestTranslation: 'too many',
      lemma: 'demasiado',
      LemmaMeanings: ['too much', 'too many', 'excessive', 'overly', 'excessively']
    },
    'cosas': {
      wordNumber: 76,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Plural',
      meanings: ['things', 'objects', 'matters'],
      bestTranslation: 'things',
      lemma: 'cosa',
      LemmaMeanings: ['thing', 'object', 'matter', 'affair', 'issue']
    },
    'él': {
      wordNumber: 77,
      frequency: 1,
      partOfSpeech: 'Pronoun',
      morphology: 'Personal Third Person Masculine Singular',
      meanings: ['he', 'him', 'it'],
      bestTranslation: 'he',
      lemma: 'él',
      LemmaMeanings: ['he', 'him', 'it', 'himself', 'that one']
    },
    'cámara': {
      wordNumber: 78,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['camera', 'chamber'],
      bestTranslation: 'camera',
      lemma: 'cámara',
      LemmaMeanings: ['camera', 'chamber', 'room', 'department', 'parliament']
    },
    'fotográfica': {
      wordNumber: 79,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Feminine Singular',
      meanings: ['photographic'],
      bestTranslation: 'photographic',
      lemma: 'fotográfico',
      LemmaMeanings: ['photographic', 'pictorial', 'camera-related', 'photography', 'picture']
    },
    'libreta': {
      wordNumber: 80,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['notebook', 'notepad'],
      bestTranslation: 'notebook',
      lemma: 'libreta',
      LemmaMeanings: ['notebook', 'notepad', 'small book', 'pad', 'binder']
    },
    'apuntes': {
      wordNumber: 81,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Plural',
      meanings: ['notes', 'annotations'],
      bestTranslation: 'notes',
      lemma: 'apunte',
      LemmaMeanings: ['note', 'annotation', 'sketch', 'jotting', 'memorandum']
    },
    'bruce': {
      wordNumber: 82,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Proper Noun',
      meanings: ['Bruce'],
      bestTranslation: 'Bruce',
      lemma: 'Bruce',
      LemmaMeanings: ['Bruce', 'male name', 'first name', 'English name', 'Bruce Chatwin']
    },
    'he': {
      wordNumber: 83,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Present Perfect Auxiliary First Person Singular',
      meanings: ['I have'],
      bestTranslation: 'I have',
      lemma: 'haber',
      LemmaMeanings: ['to have', 'to exist', 'to be', 'to occur', 'auxiliary verb']
    },
    'aborrecido': {
      wordNumber: 84,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Past Participle',
      meanings: ['hated', 'detested', 'abhorred'],
      bestTranslation: 'hated',
      lemma: 'aborrecer',
      LemmaMeanings: ['to hate', 'to detest', 'to abhor', 'to loathe', 'to despise']
    },
    'hacen': {
      wordNumber: 85,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Present Indicative Third Person Plural',
      meanings: ['they do', 'they make'],
      bestTranslation: 'they make',
      lemma: 'hacer',
      LemmaMeanings: ['to do', 'to make', 'to create', 'to cause', 'to perform']
    },
    'rayas': {
      wordNumber: 86,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Plural',
      meanings: ['lines', 'stripes', 'streaks'],
      bestTranslation: 'lines',
      lemma: 'raya',
      LemmaMeanings: ['line', 'stripe', 'streak', 'dash', 'underline']
    },
    'anotaciones': {
      wordNumber: 87,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Plural',
      meanings: ['notes', 'annotations', 'notations'],
      bestTranslation: 'annotations',
      lemma: 'anotación',
      LemmaMeanings: ['annotation', 'note', 'notation', 'comment', 'remark']
    },
    'libros': {
      wordNumber: 88,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Plural',
      meanings: ['books', 'volumes'],
      bestTranslation: 'books',
      lemma: 'libro',
      LemmaMeanings: ['book', 'volume', 'work', 'tome', 'publication']
    },
    'aquél': {
      wordNumber: 89,
      frequency: 1,
      partOfSpeech: 'Demonstrative Pronoun',
      morphology: 'Masculine Singular',
      meanings: ['that one', 'that'],
      bestTranslation: 'that one',
      lemma: 'aquel',
      LemmaMeanings: ['that', 'that one', 'the former', 'those', 'that person']
    },
    'lleno': {
      wordNumber: 90,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Masculine Singular',
      meanings: ['full', 'filled', 'complete'],
      bestTranslation: 'full',
      lemma: 'lleno',
      LemmaMeanings: ['full', 'filled', 'complete', 'crowded', 'packed']
    },
    'subrayados': {
      wordNumber: 91,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Plural',
      meanings: ['underlinings', 'underlines', 'highlights'],
      bestTranslation: 'underlines',
      lemma: 'subrayado',
      LemmaMeanings: ['underlining', 'underline', 'highlight', 'emphasis', 'stress']
    },
    'signos': {
      wordNumber: 92,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Plural',
      meanings: ['signs', 'symbols', 'marks'],
      bestTranslation: 'signs',
      lemma: 'signo',
      LemmaMeanings: ['sign', 'symbol', 'mark', 'signal', 'token']
    },
    'exclamación': {
      wordNumber: 93,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['exclamation'],
      bestTranslation: 'exclamation',
      lemma: 'exclamación',
      LemmaMeanings: ['exclamation', 'outcry', 'outburst', 'ejaculation', 'interjection']
    },
    'fueron': {
      wordNumber: 94,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Preterite Indicative Third Person Plural',
      meanings: ['they were', 'they went'],
      bestTranslation: 'they were',
      lemma: 'ser/ir',
      LemmaMeanings: ['to be/to go', 'to exist', 'to move', 'to travel', 'to proceed']
    },
    'aumento': {
      wordNumber: 95,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['increase', 'growth', 'rise'],
      bestTranslation: 'increase',
      lemma: 'aumento',
      LemmaMeanings: ['increase', 'growth', 'rise', 'increment', 'augmentation']
    },
    'tres': {
      wordNumber: 96,
      frequency: 1,
      partOfSpeech: 'Numeral',
      morphology: 'Cardinal',
      meanings: ['three'],
      bestTranslation: 'three',
      lemma: 'tres',
      LemmaMeanings: ['three', 'trio', 'triple', 'triad', 'third']
    },
    'lecturas': {
      wordNumber: 97,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Plural',
      meanings: ['readings', 'lectures', 'interpretations'],
      bestTranslation: 'readings',
      lemma: 'lectura',
      LemmaMeanings: ['reading', 'lecture', 'interpretation', 'perusal', 'literacy']
    },
    'leerlo': {
      wordNumber: 98,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Infinitive with Direct Object Pronoun',
      meanings: ['to read it'],
      bestTranslation: 'to read it',
      lemma: 'leer',
      LemmaMeanings: ['to read', 'to interpret', 'to decipher', 'to peruse', 'to study']
    },
    'cuarta': {
      wordNumber: 99,
      frequency: 1,
      partOfSpeech: 'Adjective/Numeral',
      morphology: 'Feminine Singular Ordinal',
      meanings: ['fourth'],
      bestTranslation: 'fourth',
      lemma: 'cuarto',
      LemmaMeanings: ['fourth', 'quarter', 'room', 'chamber', 'quart']
    },
    'durante': {
      wordNumber: 100,
      frequency: 1,
      partOfSpeech: 'Preposition',
      morphology: '',
      meanings: ['during', 'throughout', 'for'],
      bestTranslation: 'during',
      lemma: 'durante',
      LemmaMeanings: ['during', 'throughout', 'for', 'while', 'in the course of']
    },
    'vuelo': {
      wordNumber: 101,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['flight', 'flying'],
      bestTranslation: 'flight',
      lemma: 'vuelo',
      LemmaMeanings: ['flight', 'flying', 'air travel', 'soaring', 'aviation']
    },
    'santiago': {
      wordNumber: 102,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Proper Noun',
      meanings: ['Santiago'],
      bestTranslation: 'Santiago',
      lemma: 'Santiago',
      LemmaMeanings: ['Santiago', 'Saint James', 'capital of Chile', 'Chilean city', 'Saint Iago']
    },
    'quise': {
      wordNumber: 103,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Preterite Indicative First Person Singular',
      meanings: ['I wanted', 'I wished', 'I tried to'],
      bestTranslation: 'I wanted',
      lemma: 'querer',
      LemmaMeanings: ['to want', 'to wish', 'to love', 'to desire', 'to try to']
    },
    'regresar': {
      wordNumber: 104,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Infinitive',
      meanings: ['to return', 'to go back', 'to come back'],
      bestTranslation: 'to return',
      lemma: 'regresar',
      LemmaMeanings: ['to return', 'to go back', 'to come back', 'to turn back', 'to retreat']
    },
    'tuve': {
      wordNumber: 105,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Preterite Indicative First Person Singular',
      meanings: ['I had', 'I experienced', 'I got'],
      bestTranslation: 'I had',
      lemma: 'tener',
      LemmaMeanings: ['to have', 'to possess', 'to hold', 'to keep', 'to own']
    },
    'ganas': {
      wordNumber: 106,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Plural',
      meanings: ['desires', 'wishes', 'urges'],
      bestTranslation: 'desires',
      lemma: 'gana',
      LemmaMeanings: ['desire', 'wish', 'urge', 'inclination', 'appetite']
    },
    'determinación': {
      wordNumber: 107,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['determination', 'decision', 'resolve'],
      bestTranslation: 'determination',
      lemma: 'determinación',
      LemmaMeanings: ['determination', 'decision', 'resolve', 'resolution', 'willpower']
    },
    'pesó': {
      wordNumber: 108,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Preterite Indicative Third Person Singular',
      meanings: ['weighed', 'outweighed', 'mattered'],
      bestTranslation: 'weighed',
      lemma: 'pesar',
      LemmaMeanings: ['to weigh', 'to matter', 'to be heavy', 'to regret', 'to be important']
    },
    'miedo': {
      wordNumber: 109,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['fear', 'dread', 'fright'],
      bestTranslation: 'fear',
      lemma: 'miedo',
      LemmaMeanings: ['fear', 'dread', 'fright', 'terror', 'alarm']
    },
    'reencontrarme': {
      wordNumber: 110,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Infinitive Reflexive',
      meanings: ['to meet again', 'to reunite with', 'to find myself again'],
      bestTranslation: 'to meet again',
      lemma: 'reencontrar',
      LemmaMeanings: ['to meet again', 'to reunite', 'to rediscover', 'to find again', 'to reconnect']
    },
    'con': {
      wordNumber: 111,
      frequency: 1,
      partOfSpeech: 'Preposition',
      morphology: '',
      meanings: ['with', 'along with'],
      bestTranslation: 'with',
      lemma: 'con',
      LemmaMeanings: ['with', 'along with', 'by means of', 'together with', 'using']
    },
    'mi': {
      wordNumber: 112,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Possessive First Person Singular',
      meanings: ['my', 'mine'],
      bestTranslation: 'my',
      lemma: 'mi',
      LemmaMeanings: ['my', 'mine', 'my own', 'of mine', 'belonging to me']
    },
    'hermano': {
      wordNumber: 113,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['brother', 'sibling'],
      bestTranslation: 'brother',
      lemma: 'hermano',
      LemmaMeanings: ['brother', 'sibling', 'male sibling', 'brotherhood', 'fraternity']
    },
    'amigos': {
      wordNumber: 114,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Plural',
      meanings: ['friends', 'pals'],
      bestTranslation: 'friends',
      lemma: 'amigo',
      LemmaMeanings: ['friend', 'pal', 'buddy', 'companion', 'ally']
    },
    'tengo': {
      wordNumber: 115,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Present Indicative First Person Singular',
      meanings: ['I have', 'I own', 'I possess'],
      bestTranslation: 'I have',
      lemma: 'tener',
      LemmaMeanings: ['to have', 'to possess', 'to hold', 'to keep', 'to own']
    },
    'transformaron': {
      wordNumber: 116,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Preterite Indicative Third Person Plural',
      meanings: ['transformed', 'changed', 'converted'],
      bestTranslation: 'transformed',
      lemma: 'transformar',
      LemmaMeanings: ['to transform', 'to change', 'to convert', 'to turn into', 'to modify']
    },
    'promesa': {
      wordNumber: 117,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['promise', 'pledge', 'vow'],
      bestTranslation: 'promise',
      lemma: 'promesa',
      LemmaMeanings: ['promise', 'pledge', 'vow', 'commitment', 'undertaking']
    },
    'tan': {
      wordNumber: 118,
      frequency: 1,
      partOfSpeech: 'Adverb',
      morphology: '',
      meanings: ['so', 'such', 'as'],
      bestTranslation: 'so',
      lemma: 'tan',
      LemmaMeanings: ['so', 'such', 'as', 'that much', 'to that extent']
    },
    'menos': {
      wordNumber: 119,
      frequency: 1,
      partOfSpeech: 'Adverb',
      morphology: '',
      meanings: ['less', 'minus', 'fewer'],
      bestTranslation: 'less',
      lemma: 'menos',
      LemmaMeanings: ['less', 'minus', 'fewer', 'not as much', 'least']
    },
    'demasiados': {
      wordNumber: 120,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Masculine Plural',
      meanings: ['too many', 'excessive'],
      bestTranslation: 'too many',
      lemma: 'demasiado',
      LemmaMeanings: ['too much', 'excessive', 'too many', 'overly', 'undue']
    },
    'vagando': {
      wordNumber: 121,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Gerund',
      meanings: ['wandering', 'roaming', 'drifting'],
      bestTranslation: 'wandering',
      lemma: 'vagar',
      LemmaMeanings: ['to wander', 'to roam', 'to drift', 'to rove', 'to ramble']
    },
    'sin': {
      wordNumber: 122,
      frequency: 1,
      partOfSpeech: 'Preposition',
      morphology: '',
      meanings: ['without', 'lacking'],
      bestTranslation: 'without',
      lemma: 'sin',
      LemmaMeanings: ['without', 'lacking', 'devoid of', 'minus', 'absent']
    },
    'rumbo': {
      wordNumber: 123,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['course', 'direction', 'heading'],
      bestTranslation: 'direction',
      lemma: 'rumbo',
      LemmaMeanings: ['direction', 'course', 'heading', 'bearing', 'route']
    },
    'fijo': {
      wordNumber: 124,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Masculine Singular',
      meanings: ['fixed', 'steady', 'firm'],
      bestTranslation: 'fixed',
      lemma: 'fijo',
      LemmaMeanings: ['fixed', 'steady', 'firm', 'stable', 'permanent']
    },
    'detenerme': {
      wordNumber: 125,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Infinitive Reflexive',
      meanings: ['to stop myself', 'to halt', 'to pause'],
      bestTranslation: 'to stop myself',
      lemma: 'detener',
      LemmaMeanings: ['to stop', 'to halt', 'to arrest', 'to detain', 'to pause']
    },
    'aconsejaban': {
      wordNumber: 126,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Imperfect Indicative Third Person Plural',
      meanings: ['they advised', 'they recommended', 'they counseled'],
      bestTranslation: 'they advised',
      lemma: 'aconsejar',
      LemmaMeanings: ['to advise', 'to counsel', 'to recommend', 'to suggest', 'to guide']
    },
    'pequeño': {
      wordNumber: 127,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Masculine Singular',
      meanings: ['small', 'little', 'tiny'],
      bestTranslation: 'small',
      lemma: 'pequeño',
      LemmaMeanings: ['small', 'little', 'tiny', 'minor', 'slight']
    },
    'pueblo': {
      wordNumber: 128,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['town', 'village', 'people'],
      bestTranslation: 'town',
      lemma: 'pueblo',
      LemmaMeanings: ['town', 'village', 'people', 'nation', 'populace']
    },
    'pescadores': {
      wordNumber: 129,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Plural',
      meanings: ['fishermen', 'anglers'],
      bestTranslation: 'fishermen',
      lemma: 'pescador',
      LemmaMeanings: ['fisherman', 'angler', 'fisher', 'fishing person', 'one who fishes']
    },
    'creta': {
      wordNumber: 130,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Proper Noun',
      meanings: ['Crete'],
      bestTranslation: 'Crete',
      lemma: 'Creta',
      LemmaMeanings: ['Crete', 'Greek island', 'Mediterranean island', 'Cretan island', 'Kriti']
    },
    'ierápetras': {
      wordNumber: 131,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Proper Noun',
      meanings: ['Ierapetra'],
      bestTranslation: 'Ierapetra',
      lemma: 'Ierápetras',
      LemmaMeanings: ['Ierapetra', 'city in Crete', 'Cretan town', 'coastal town', 'Greek municipality']
    },
    'apacible': {
      wordNumber: 132,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Singular',
      meanings: ['peaceful', 'calm', 'serene'],
      bestTranslation: 'peaceful',
      lemma: 'apacible',
      LemmaMeanings: ['peaceful', 'calm', 'serene', 'quiet', 'gentle']
    },
    'ciudad': {
      wordNumber: 133,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['city', 'town'],
      bestTranslation: 'city',
      lemma: 'ciudad',
      LemmaMeanings: ['city', 'town', 'metropolis', 'urban area', 'municipality']
    },
    'asturiana': {
      wordNumber: 134,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Feminine Singular',
      meanings: ['Asturian', 'from Asturias'],
      bestTranslation: 'Asturian',
      lemma: 'asturiano',
      LemmaMeanings: ['Asturian', 'from Asturias', 'of Asturias', 'pertaining to Asturias', 'native of Asturias']
    },
    'villa': {
      wordNumber: 135,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['villa', 'town'],
      bestTranslation: 'villa',
      lemma: 'villa',
      LemmaMeanings: ['villa', 'town', 'resort', 'mansion', 'country house']
    },
    'viciosa': {
      wordNumber: 136,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Feminine Singular',
      meanings: ['vicious', 'luxuriant'],
      bestTranslation: 'Viciosa',
      lemma: 'vicioso',
      LemmaMeanings: ['vicious', 'luxuriant', 'lush', 'corrupt', 'addictive']
    },
    'algún': {
      wordNumber: 137,
      frequency: 1,
      partOfSpeech: 'Adjective/Determiner',
      morphology: 'Masculine Singular',
      meanings: ['some', 'any'],
      bestTranslation: 'some',
      lemma: 'alguno',
      LemmaMeanings: ['some', 'any', 'a certain', 'someone', 'somewhat']
    },
    'día': {
      wordNumber: 138,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['day'],
      bestTranslation: 'day',
      lemma: 'día',
      LemmaMeanings: ['day', 'daytime', 'daylight', 'date', 'daily period']
    },
    'cayo': {
      wordNumber: 139,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Preterite Indicative Third Person Singular',
      meanings: ['fell', 'dropped', 'fell down'],
      bestTranslation: 'fell',
      lemma: 'caer',
      LemmaMeanings: ['to fall', 'to drop', 'to descend', 'to tumble', 'to collapse']
    },
    'mis': {
      wordNumber: 140,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Possessive First Person Plural',
      meanings: ['my'],
      bestTranslation: 'my',
      lemma: 'mi',
      LemmaMeanings: ['my', 'mine', 'my own', 'of mine', 'belonging to me']
    },
    'manos': {
      wordNumber: 141,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Plural',
      meanings: ['hands'],
      bestTranslation: 'hands',
      lemma: 'mano',
      LemmaMeanings: ['hand', 'palm', 'handwriting', 'worker', 'manual skill']
    },
    'para': {
      wordNumber: 142,
      frequency: 1,
      partOfSpeech: 'Preposition',
      morphology: '',
      meanings: ['for', 'in order to', 'to'],
      bestTranslation: 'for',
      lemma: 'para',
      LemmaMeanings: ['for', 'in order to', 'to', 'towards', 'by']
    },
    'devolverme': {
      wordNumber: 143,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Infinitive Reflexive',
      meanings: ['to return me', 'to bring me back', 'to restore me'],
      bestTranslation: 'to return me',
      lemma: 'devolver',
      LemmaMeanings: ['to return', 'to give back', 'to restore', 'to repay', 'to refund']
    },
    'olvidado': {
      wordNumber: 144,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Masculine Singular',
      meanings: ['forgotten', 'overlooked', 'neglected'],
      bestTranslation: 'forgotten',
      lemma: 'olvidado',
      LemmaMeanings: ['forgotten', 'overlooked', 'neglected', 'unremembered', 'disregarded']
    },
    'esperando': {
      wordNumber: 145,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Gerund',
      meanings: ['waiting', 'expecting', 'hoping'],
      bestTranslation: 'waiting',
      lemma: 'esperar',
      LemmaMeanings: ['to wait', 'to expect', 'to hope', 'to anticipate', 'to look forward to']
    },
    'leer': {
      wordNumber: 146,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Infinitive',
      meanings: ['to read'],
      bestTranslation: 'to read',
      lemma: 'leer',
      LemmaMeanings: ['to read', 'to peruse', 'to interpret', 'to decipher', 'to study']
    },
    'primera': {
      wordNumber: 147,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Feminine Singular Ordinal',
      meanings: ['first', 'primary'],
      bestTranslation: 'first',
      lemma: 'primero',
      LemmaMeanings: ['first', 'primary', 'initial', 'principal', 'leading']
    },
    'entró': {
      wordNumber: 148,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Preterite Indicative Third Person Singular',
      meanings: ['entered', 'came in', 'went in'],
      bestTranslation: 'entered',
      lemma: 'entrar',
      LemmaMeanings: ['to enter', 'to come in', 'to go in', 'to get in', 'to access']
    },
    'desesperación': {
      wordNumber: 149,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['desperation', 'despair', 'hopelessness'],
      bestTranslation: 'desperation',
      lemma: 'desesperación',
      LemmaMeanings: ['desperation', 'despair', 'hopelessness', 'anguish', 'distress']
    },
    'volver': {
      wordNumber: 150,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Infinitive',
      meanings: ['to return', 'to come back', 'to turn'],
      bestTranslation: 'to return',
      lemma: 'volver',
      LemmaMeanings: ['to return', 'to come back', 'to go back', 'to turn', 'to revert']
    },
    'solo': {
      wordNumber: 151,
      frequency: 1,
      partOfSpeech: 'Adjective/Adverb',
      morphology: 'Masculine Singular',
      meanings: ['alone', 'only', 'just', 'solely'],
      bestTranslation: 'alone',
      lemma: 'solo',
      LemmaMeanings: ['alone', 'only', 'just', 'solely', 'single']
    },
    'esos': {
      wordNumber: 152,
      frequency: 1,
      partOfSpeech: 'Adjective/Pronoun',
      morphology: 'Demonstrative Masculine Plural',
      meanings: ['those', 'these'],
      bestTranslation: 'those',
      lemma: 'ese',
      LemmaMeanings: ['that', 'those', 'that one', 'the ones', 'such']
    },
    'lugares': {
      wordNumber: 153,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Plural',
      meanings: ['places', 'locations', 'spots'],
      bestTranslation: 'places',
      lemma: 'lugar',
      LemmaMeanings: ['place', 'location', 'spot', 'site', 'position']
    },
    'habían': {
      wordNumber: 154,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Imperfect Indicative Third Person Plural',
      meanings: ['they had', 'there were'],
      bestTranslation: 'they had',
      lemma: 'haber',
      LemmaMeanings: ['to have', 'to exist', 'to be', 'to occur', 'auxiliary verb']
    },
    'sido': {
      wordNumber: 155,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Past Participle',
      meanings: ['been'],
      bestTranslation: 'been',
      lemma: 'ser',
      LemmaMeanings: ['to be', 'to exist', 'to happen', 'to occur', 'to take place']
    },
    'rechazo': {
      wordNumber: 156,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['rejection', 'refusal', 'denial'],
      bestTranslation: 'rejection',
      lemma: 'rechazo',
      LemmaMeanings: ['rejection', 'refusal', 'denial', 'repudiation', 'rebuff']
    },
    'idea': {
      wordNumber: 157,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['idea', 'thought', 'concept'],
      bestTranslation: 'idea',
      lemma: 'idea',
      LemmaMeanings: ['idea', 'thought', 'concept', 'notion', 'view']
    },
    'viajar': {
      wordNumber: 158,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Infinitive',
      meanings: ['to travel', 'to journey', 'to voyage'],
      bestTranslation: 'to travel',
      lemma: 'viajar',
      LemmaMeanings: ['to travel', 'to journey', 'to voyage', 'to tour', 'to trip']
    },
    'podían': {
      wordNumber: 159,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Imperfect Indicative Third Person Plural',
      meanings: ['they could', 'they were able to'],
      bestTranslation: 'they could',
      lemma: 'poder',
      LemmaMeanings: ['to be able to', 'can', 'to have power', 'to be capable', 'to be possible']
    },
    'crear': {
      wordNumber: 160,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Infinitive',
      meanings: ['to create', 'to make', 'to produce'],
      bestTranslation: 'to create',
      lemma: 'crear',
      LemmaMeanings: ['to create', 'to make', 'to produce', 'to generate', 'to invent']
    },
    'algo': {
      wordNumber: 161,
      frequency: 1,
      partOfSpeech: 'Pronoun/Adverb',
      morphology: '',
      meanings: ['something', 'somewhat', 'a little'],
      bestTranslation: 'something',
      lemma: 'algo',
      LemmaMeanings: ['something', 'somewhat', 'a little', 'anything', 'rather']
    },
    'nuevo': {
      wordNumber: 162,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Masculine Singular',
      meanings: ['new', 'fresh', 'novel'],
      bestTranslation: 'new',
      lemma: 'nuevo',
      LemmaMeanings: ['new', 'fresh', 'novel', 'recent', 'modern']
    },
    'sentí': {
      wordNumber: 163,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Preterite Indicative First Person Singular',
      meanings: ['I felt', 'I sensed', 'I perceived'],
      bestTranslation: 'I felt',
      lemma: 'sentir',
      LemmaMeanings: ['to feel', 'to sense', 'to perceive', 'to regret', 'to experience']
    },
    'triste': {
      wordNumber: 164,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Singular',
      meanings: ['sad', 'unhappy', 'sorrowful'],
      bestTranslation: 'sad',
      lemma: 'triste',
      LemmaMeanings: ['sad', 'unhappy', 'sorrowful', 'melancholy', 'gloomy']
    },
    'empecé': {
      wordNumber: 165,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Preterite Indicative First Person Singular',
      meanings: ['I began', 'I started', 'I commenced'],
      bestTranslation: 'I began',
      lemma: 'empezar',
      LemmaMeanings: ['to begin', 'to start', 'to commence', 'to initiate', 'to launch']
    },
    'caminar': {
      wordNumber: 166,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Infinitive',
      meanings: ['to walk', 'to trek', 'to hike'],
      bestTranslation: 'to walk',
      lemma: 'caminar',
      LemmaMeanings: ['to walk', 'to trek', 'to hike', 'to stroll', 'to go on foot']
    },
    'seguir': {
      wordNumber: 167,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Infinitive',
      meanings: ['to follow', 'to continue', 'to pursue'],
      bestTranslation: 'to follow',
      lemma: 'seguir',
      LemmaMeanings: ['to follow', 'to continue', 'to pursue', 'to proceed', 'to keep on']
    },
    'mismo': {
      wordNumber: 168,
      frequency: 1,
      partOfSpeech: 'Adjective/Pronoun',
      morphology: 'Masculine Singular',
      meanings: ['same', 'self', 'very'],
      bestTranslation: 'same',
      lemma: 'mismo',
      LemmaMeanings: ['same', 'self', 'very', 'identical', 'exact']
    },
    'camino': {
      wordNumber: 169,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['road', 'path', 'way'],
      bestTranslation: 'road',
      lemma: 'camino',
      LemmaMeanings: ['road', 'path', 'way', 'route', 'journey']
    },
    'llevó': {
      wordNumber: 170,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Preterite Indicative Third Person Singular',
      meanings: ['took', 'carried', 'led'],
      bestTranslation: 'took',
      lemma: 'llevar',
      LemmaMeanings: ['to take', 'to carry', 'to lead', 'to bring', 'to wear']
    },
    'sur': {
      wordNumber: 171,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['south', 'southern region'],
      bestTranslation: 'south',
      lemma: 'sur',
      LemmaMeanings: ['south', 'southern region', 'southern direction', 'southern area', 'down']
    },
    'país': {
      wordNumber: 172,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['country', 'nation', 'land'],
      bestTranslation: 'country',
      lemma: 'país',
      LemmaMeanings: ['country', 'nation', 'land', 'homeland', 'state']
    },
    'atravesé': {
      wordNumber: 173,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Preterite Indicative First Person Singular',
      meanings: ['I crossed', 'I traversed', 'I went through'],
      bestTranslation: 'I crossed',
      lemma: 'atravesar',
      LemmaMeanings: ['to cross', 'to traverse', 'to go through', 'to pass through', 'to pierce']
    },
    'frontera': {
      wordNumber: 174,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['border', 'frontier', 'boundary'],
      bestTranslation: 'border',
      lemma: 'frontera',
      LemmaMeanings: ['border', 'frontier', 'boundary', 'borderline', 'limit']
    },
    'argentina': {
      wordNumber: 175,
      frequency: 1,
      partOfSpeech: 'Noun/Adjective',
      morphology: 'Feminine Singular',
      meanings: ['Argentina', 'Argentinian'],
      bestTranslation: 'Argentina',
      lemma: 'Argentina',
      LemmaMeanings: ['Argentina', 'Argentine Republic', 'Argentinian', 'Argentine', 'from Argentina']
    },
    'antes': {
      wordNumber: 176,
      frequency: 1,
      partOfSpeech: 'Adverb',
      morphology: '',
      meanings: ['before', 'earlier', 'previously'],
      bestTranslation: 'before',
      lemma: 'antes',
      LemmaMeanings: ['before', 'earlier', 'previously', 'formerly', 'sooner']
    },
    'escribir': {
      wordNumber: 177,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Infinitive',
      meanings: ['to write', 'to compose', 'to author'],
      bestTranslation: 'to write',
      lemma: 'escribir',
      LemmaMeanings: ['to write', 'to compose', 'to author', 'to type', 'to draft']
    },
    'carta': {
      wordNumber: 178,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['letter', 'card', 'menu'],
      bestTranslation: 'letter',
      lemma: 'carta',
      LemmaMeanings: ['letter', 'card', 'menu', 'map', 'charter']
    },
    'hijos': {
      wordNumber: 179,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Plural',
      meanings: ['children', 'sons'],
      bestTranslation: 'children',
      lemma: 'hijo',
      LemmaMeanings: ['son', 'child', 'descendant', 'offspring', 'progeny']
    },
    'vi': {
      wordNumber: 180,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Preterite Indicative First Person Singular',
      meanings: ['I saw', 'I watched', 'I viewed'],
      bestTranslation: 'I saw',
      lemma: 'ver',
      LemmaMeanings: ['to see', 'to watch', 'to view', 'to witness', 'to observe']
    },
    'montañas': {
      wordNumber: 181,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Plural',
      meanings: ['mountains', 'mounts'],
      bestTranslation: 'mountains',
      lemma: 'montaña',
      LemmaMeanings: ['mountain', 'mount', 'peak', 'hill', 'highlands']
    },
    'belleza': {
      wordNumber: 182,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['beauty', 'loveliness'],
      bestTranslation: 'beauty',
      lemma: 'belleza',
      LemmaMeanings: ['beauty', 'loveliness', 'attractiveness', 'fairness', 'splendor']
    },
    'increíble': {
      wordNumber: 183,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Singular',
      meanings: ['incredible', 'unbelievable', 'amazing'],
      bestTranslation: 'incredible',
      lemma: 'increíble',
      LemmaMeanings: ['incredible', 'unbelievable', 'amazing', 'astonishing', 'extraordinary']
    },
    'quedé': {
      wordNumber: 184,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Preterite Indicative First Person Singular',
      meanings: ['I stayed', 'I remained', 'I was left'],
      bestTranslation: 'I stayed',
      lemma: 'quedar',
      LemmaMeanings: ['to stay', 'to remain', 'to be left', 'to agree', 'to meet']
    },
    'maravillado': {
      wordNumber: 185,
      frequency: 1,
      partOfSpeech: 'Adjective/Verb',
      morphology: 'Past Participle Masculine Singular',
      meanings: ['amazed', 'astonished', 'marveled'],
      bestTranslation: 'amazed',
      lemma: 'maravillar',
      LemmaMeanings: ['to amaze', 'to astonish', 'to wonder', 'to marvel', 'to dazzle']
    },
    'ahora': {
      wordNumber: 186,
      frequency: 1,
      partOfSpeech: 'Adverb',
      morphology: '',
      meanings: ['now', 'nowadays', 'at present'],
      bestTranslation: 'now',
      lemma: 'ahora',
      LemmaMeanings: ['now', 'nowadays', 'at present', 'currently', 'at this time']
    },
    'entiendo': {
      wordNumber: 187,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Present Indicative First Person Singular',
      meanings: ['I understand', 'I comprehend', 'I get it'],
      bestTranslation: 'I understand',
      lemma: 'entender',
      LemmaMeanings: ['to understand', 'to comprehend', 'to grasp', 'to know', 'to perceive']
    },
    'pasión': {
      wordNumber: 188,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['passion', 'fervor', 'intensity'],
      bestTranslation: 'passion',
      lemma: 'pasión',
      LemmaMeanings: ['passion', 'fervor', 'intensity', 'desire', 'enthusiasm']
    },
    'viajes': {
      wordNumber: 189,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Plural',
      meanings: ['travels', 'trips', 'journeys'],
      bestTranslation: 'travels',
      lemma: 'viaje',
      LemmaMeanings: ['travel', 'trip', 'journey', 'voyage', 'excursion']
    },
    'descubrir': {
      wordNumber: 190,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Infinitive',
      meanings: ['to discover', 'to find out', 'to uncover'],
      bestTranslation: 'to discover',
      lemma: 'descubrir',
      LemmaMeanings: ['to discover', 'to find out', 'to uncover', 'to reveal', 'to detect']
    },
    'nuevos': {
      wordNumber: 191,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Masculine Plural',
      meanings: ['new', 'novel', 'fresh'],
      bestTranslation: 'new',
      lemma: 'nuevo',
      LemmaMeanings: ['new', 'novel', 'fresh', 'recent', 'modern']
    },
    'horizontes': {
      wordNumber: 192,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Plural',
      meanings: ['horizons', 'skylines', 'prospects'],
      bestTranslation: 'horizons',
      lemma: 'horizonte',
      LemmaMeanings: ['horizon', 'skyline', 'prospect', 'outlook', 'limit']
    },
    'si': {
      wordNumber: 193,
      frequency: 1,
      partOfSpeech: 'Conjunction',
      morphology: '',
      meanings: ['if', 'whether', 'in case'],
      bestTranslation: 'if',
      lemma: 'si',
      LemmaMeanings: ['if', 'whether', 'in case', 'provided that', 'assuming that']
    },
    'pudiera': {
      wordNumber: 194,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Imperfect Subjunctive First/Third Person Singular',
      meanings: ['could', 'would be able to'],
      bestTranslation: 'could',
      lemma: 'poder',
      LemmaMeanings: ['to be able to', 'can', 'to have power', 'to be capable', 'to be possible']
    },
    'atrás': {
      wordNumber: 195,
      frequency: 1,
      partOfSpeech: 'Adverb',
      morphology: '',
      meanings: ['behind', 'back', 'backwards'],
      bestTranslation: 'behind',
      lemma: 'atrás',
      LemmaMeanings: ['behind', 'back', 'backwards', 'in the past', 'previously']
    },
    'nunca': {
      wordNumber: 196,
      frequency: 1,
      partOfSpeech: 'Adverb',
      morphology: '',
      meanings: ['never', 'not ever'],
      bestTranslation: 'never',
      lemma: 'nunca',
      LemmaMeanings: ['never', 'not ever', 'at no time', 'not once', 'on no occasion']
    },
    'regresaría': {
      wordNumber: 197,
      frequency: 1,
      partOfSpeech: 'Verb',
      morphology: 'Conditional Indicative First/Third Person Singular',
      meanings: ['would return', 'would come back'],
      bestTranslation: 'would return',
      lemma: 'regresar',
      LemmaMeanings: ['to return', 'to come back', 'to go back', 'to turn back', 'to retreat']
    },
    'casa': {
      wordNumber: 198,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Feminine Singular',
      meanings: ['house', 'home', 'residence'],
      bestTranslation: 'house',
      lemma: 'casa',
      LemmaMeanings: ['house', 'home', 'residence', 'dwelling', 'household']
    },
    'verdadero': {
      wordNumber: 199,
      frequency: 1,
      partOfSpeech: 'Adjective',
      morphology: 'Masculine Singular',
      meanings: ['true', 'real', 'genuine'],
      bestTranslation: 'true',
      lemma: 'verdadero',
      LemmaMeanings: ['true', 'real', 'genuine', 'authentic', 'actual']
    },
    'hogar': {
      wordNumber: 200,
      frequency: 1,
      partOfSpeech: 'Noun',
      morphology: 'Masculine Singular',
      meanings: ['home', 'household', 'hearth'],
      bestTranslation: 'home',
      lemma: 'hogar',
      LemmaMeanings: ['home', 'household', 'hearth', 'fireplace', 'domicile']
    }
  };
  
  export default mundo3Database;